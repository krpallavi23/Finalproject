...
namespace OnlineJobPortal
{
    public partial class RejectedJobs : System.Web.UI.Page
    {
        ...
        /// <summary>
        /// Binds the GridView with rejected job applications.
        /// </summary>
        private void BindJobApplications()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    string query = @"SELECT ApplicationID, JobTitle, CompanyName, ApplicationDate, Status 
                                     FROM JobApplication 
                                     WHERE JobSeekerID = @JobSeekerID AND Status = 'Rejected'
            ...
            catch (Exception ex)
            {
                ...
                lblMessage.Text = "An error occurred while loading your rejected jobs. Please try again later.";
            }
        }
        ...
        private void BindJobPostingsDropdown()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    string query = @"SELECT jp.JobID, jp.Title 
                                     FROM JobPosting jp
                                     INNER JOIN JobApplication ja ON jp.JobID = ja.JobID
                                     WHERE ja.JobSeekerID = @JobSeekerID AND ja.Status = 'Rejected'";
