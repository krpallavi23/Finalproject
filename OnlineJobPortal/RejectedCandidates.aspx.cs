...
{
    public partial class RejectedCandidates : Page
    {
        ...
        private void LoadRejectedCandidates(int jobId)
        {
            List<MatchingCandidate> rejectedCandidates = new List<MatchingCandidate>();
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = @"SELECT JS.JobSeekerID, JS.FirstName, JS.LastName, JS.YearsOfExperience, JS.ResumePath, JA.Status
                                 ...
                                 WHERE JA.JobID = @JobID AND JA.Status = 'Rejected'";
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                rejectedCandidates.Add(new MatchingCandidate
                                {
                                    Candidate = candidate,
            ...
            gvRejectedCandidates.DataSource = rejectedCandidates;
            gvRejectedCandidates.DataBind();
        }
        ...
        protected void gvRejectedCandidates_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            lblError.Text = "";
        ...
        protected void gvRejectedCandidates_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (dataItem != null)
                {
                    // Optional: Modify button states based on ApplicationStatus if needed
                    // Currently, since all are Rejected, buttons remain enabled
                }
            }
        }
